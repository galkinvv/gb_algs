...
namespace{
	...
	template <class PairRow>
	void TriangulateMatrix(std::vector<PairRow>& matrix, std::vector<int>& lead_columns)
	{
		lead_columns.reserve(matrix.size());
...
void SolveWithRightSideContainigSingleOne(const Field& field, const ElementMatrixContainer& matrix, std::vector<Element<Field>>& result, int max_diferent_numbers_in_coefficients)
{
	...
	std::vector<auto_unique_ptr<PairRow<Field, ExactField>>> pair_matrix;
